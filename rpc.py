...
def init():
    ...
    p.add_argument('name',
                   help='Target node name (ASCII)')
    p.add_argument('alias_name',
                   help='Target node alias name (ASCII)')
    p.add_argument('lun_name_id_pairs',
                   help="""Whitespace-separated list of LUN <name:id> pairs enclosed
    ...
    *** The Portal/Initiator Groups must be precreated ***""")
    p.add_argument('queue_depth',
                   help='Desired target queue depth', type=int)
    p.add_argument('chap_disabled',
                   help="""CHAP authentication should be disabled for this target node.
    *** Mutually exclusive with chap_required ***""", type=int)
    p.add_argument('chap_required',
                   help="""CHAP authentication should be required for this target node.
    *** Mutually exclusive with chap_disabled ***""", type=int)
    p.add_argument(
        'chap_mutual',
        help='CHAP authentication should be mutual/bidirectional.',
        type=int)
    p.add_argument('chap_auth_group',
                   help="""Authentication group ID for this target node.
    *** Authentication group must be precreated ***""", type=int)
    ...
    p.add_argument(
        'total_size',
        help='Size of null bdev in MB (int > 0)',
        type=int)
    p.add_argument('block_size', help='Block size for this bdev', type=int)
    ...
    p.add_argument(
        'flag',
        help='trace mask we want to set. (for example "debug").')
    ...
    p.add_argument(
        'flag',
        help='trace mask we want to clear. (for example "debug").')
    ...
    p.add_argument(
        'tag',
        help='Portal group tag (unique, integer > 0)',
        type=int)
    p.add_argument('portal_list', nargs=argparse.REMAINDER,
    ...
    p.add_argument(
        'tag',
        help='Initiator group tag (unique, integer > 0)',
        type=int)
    p.add_argument('initiator_list',
    ...
    p = subparsers.add_parser(
        'get_interfaces',
        help='Display current interface list')
    ...
    p.add_argument(
        'bdev_name',
        help='Blockdev name to be deleted. Example: Malloc0.')
    ...
    p.add_argument(
        "-c",
        "--core",
        help='The core Nvmf target run on',
        type=int,
        default=-1)
    p.add_argument('nqn', help='Target nqn(ASCII)')
    p.add_argument(
        'listen', help="""comma-separated list of Listen <trtype:transport_name
         traddr:address trsvcid:port_id> pairs enclosed
    in quotes.  Format:  'trtype:transport0 traddr:traddr0 trsvcid:trsvcid0,
    trtype:transport1 traddr:traddr1 trsvcid:trsvcid1' etc
    Example: 'trtype:RDMA traddr:192.168.100.8 trsvcid:4420,trtype:RDMA
    traddr:192.168.100.9 trsvcid:4420'""")
    ...
    p.add_argument("-n", "--namespaces",
                   help="""Whitespace-separated list of namespaces.
    ...
    *** The devices must pre-exist ***""")
    ...
    p = subparsers.add_parser(
        ...
        help='Remove device from vhost controller')
    ...
    p = subparsers.add_parser(
        ...
        help='Remove a vhost block controller')
    p.add_argument('ctrlr', help='controller name')
